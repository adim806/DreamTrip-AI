...
const ItineraryDisplay = () => {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className={`itinerary-container bg-[#2a3146] border border-blue-500/20 rounded-xl p-4 my-4 text-white ${isExpanded ? 'max-w-[95%]' : 'max-w-[90%]'} self-start`}
    >
      <div className="itinerary-header mb-3 border-b border-blue-400/20 pb-2">
        <h3 className="text-lg font-medium text-blue-300">Travel Itinerary for {destination}</h3>
        <p className="text-sm text-gray-300">{dateRange}</p>
      </div>
      
      <div className={`itinerary-content text-sm ${isExpanded ? '' : 'max-h-[400px] overflow-y-auto'}`}>
